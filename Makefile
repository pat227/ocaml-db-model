...
lib: $(libdir)utilities.ml $(libdir)utilities.mli $(libdir)sql_supported_types.ml $(libdir)sql_supported_types.mli $(libdir)model.ml $(libdir)model.mli $(libdir)table.ml $(libdir)table.mli
	...
	ocamlbuild -classic-display -use-ocamlfind -j 1 -tag thread -tag principal -r -package 'core mysql ppx_deriving ppx_deriving.show fieldslib ppx_fields_conv' -build-dir build -I src/lib -I src/main src/lib/model.cmxs

command: lib $(maindir)command.ml
	ocamlbuild -classic-display -use-ocamlfind -j 1 -tag thread -tag principal -r -package 'core mysql ppx_deriving ppx_deriving.show fieldslib ppx_fields_conv' -build-dir build -I src/lib -I src/main src/main/command.native
